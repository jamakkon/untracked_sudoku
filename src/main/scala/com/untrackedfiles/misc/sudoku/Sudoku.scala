...
case class Board(cells: List[Cell]) {
  ...
  def +(c: Cell): Board = {
    Board((cells.filterNot(d => d.x == c.x && d.y == c.y) :+ c).sortBy(d => (d.y, d.x)))
  }
...
object Board {
  ...
  def parse(str: String): Board = {
    ...
    Board(cells.sortBy(c => (c.y, c.x)).toList)
  }

  def prettyString(board: Board): String = {
...
object Sudoku extends LazyLogging {
...
object SudokuBenchmark extends LazyLogging {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val batch = data.par.map { sudokuString =>
      val board = Board.parse(sudokuString)
      val start: Long = System.currentTimeMillis
      val result = Sudoku.solve(board)
